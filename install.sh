...
if [ "$daemon_updated" -eq 1 ]; then
  ...
  # 未注册孤儿的兑底是看门狗的 300s 孤儿分支；不装看门狗时它没有自动兑底，
  # 只能靠重启终端（第 31 轮删掉了原本写在这里的「唯一兑底就是本次安装的
  # 这段回收」——上面第 30 轮已经判定那句是错的，却把它留在了原地，还在
  # 下面被当既成事实引用；照它复刻的人为了兑现这句话会把身份判据放宽成
  # 「命令行提及脚本名」，那正是 2026-08-13/14 两次误杀事故的形态）。
  recycled=0
  ...
  # 判据与钩子完全一致：cmdline 里 bash 之后的第一个非选项 argv 必须就是 daemon 脚本；读不到就不杀（第 32 轮订正这句：它写的是尾锚，而下面第 30/31 轮的代码走的是首 argv——同一段挂两条互斥判据，正是下一个维护者照前导注释改回去的路径）。
  ...
  # 判据收窄为「bash 之后的第一个非选项 argv」并排除任何带裸 `-c` 的外壳
  # （第 31 轮订正这句前导注释：它说的是尾锚，而紧接着的代码第 30 轮已经
  # 改成首 argv——同一段代码挂两条互斥判据，下一个维护者照前导注释一改
  # 就退回去了）。
  old_cmd=""
  old_ok=0
  ...
  # 拍拉一个新实例。而未注册孤儿的兑底只有看门狗那条 300s 分支（不装看门狗
  # 时没有自动兑底），无条件删 pid 文件等于亲手造一个（第 32 轮订正：此处原本
  # 引用的是一句第 30 轮已判伪、第 31 轮已从本文件删除的话——引用一句不存在的
  # 注释当依据，正是让下一个人为了「兑现」它而把身份判据放宽成「命令行提及
  # 脚本名」的那条路，那是 2026-08-13/14 两次误杀事故的形态）。
  still_alive=0
