...
echo "DEBUG desc_budget=$desc_budget active_col_width_sum=$active_col_width_sum active_col_count=$active_col_count" >&2
...
for ((r=0; r<row_total; r++)); do
  ...
  if [ -n "$description" ] && [ "$desc_budget" -ge 8 ]; then
    ...
    __dbg_t1=$(date +%s%N)
    echo "DEBUG row=$r desc_chars=${#description} desc_disp_len=$desc_disp_len prechk_ms=$(( (__dbg_t1-__dbg_t0)/1000000 ))" >&2
    if [ "$desc_disp_len" -gt "$desc_budget" ]; then
      ...
      __dbg_t3=$(date +%s%N)
      echo "DEBUG row=$r trunc_loop_iters=$cut_pos trunc_ms=$(( (__dbg_t3-__dbg_t2)/1000000 ))" >&2
    else
