...
- 灰`▸` + **亮紫**身份（主行是亮青，主/子一眼分）+ 灰`(类型)` + 热度`·档位`（仅显式指定）+ 状态图标 `●`运行绿/`○`排队黄/`✗`失败红/`✓`完成绿；紧随其后是**独立模型列**（青短名恒显，剥 `claude-` 前缀与日期后缀、**保留** `[1m]` 容量标记；全面板无模型则整列裁掉）
...
bash test.sh --assert   # 102 项断言（CI 用，含性能门槛与十一轮对抗审查回归组）
```
