...
- 灰`▸` + **身份按状态分色**（运行亮紫/排队黄/完成绿/失败亮红/未知亮蓝——身份是行内最宽最靠左的字段，让它直接承载状态，一眼扫左边缘就知道每个代理在干什么；主行身份是亮青，主/子仍一眼分）+ 灰`(类型)` + 热度`·档位`（仅显式指定）+ 状态图标 `●`运行绿/`○`排队黄/`✗`失败红/`✓`完成绿（形状与颜色双编码）；紧随其后是**独立模型列**（青短名恒显，剥 `claude-` 前缀与日期后缀、**保留** `[1m]` 容量标记；全面板无模型则整列裁掉）
