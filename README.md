...
| 3 | 花费 | `$`金额+`$X.X/h`速率 · `today` 今日跨会话总花费（**仅当今天还有别的会话花过钱时才显示**——只开一个会话时它与左边的金额段完全重复，故整段让位；同一笔钱在 week 里照常出现） · `week` 近 7 天总花费 · `+增/-删`行数 |
...
- **窄终端自动降级**：`$COLUMNS` <100 时只渲染**一行紧凑模式**（时钟｜模型｜末级目录｜分支｜电池｜$金额｜5h%），四行网格与其余各段整体让位——分屏/侧栏下看到一行属正常行为，不是装坏了
- **状态文件双层**：细粒度 `statusline-history.tsv`（0x1F，TAB 旧格式自动兼容，**90 分钟**窗）只养走势/速率/$每小时；today/week 走 `statusline-daily.tsv` 日聚合（单调段状态机 + 水位线增量：重放不双计、并发丢写自愈、首跑自动播种；**跨午夜按日初基线扣减**——会话累计不在午夜清零，不减基线会让 week 把午夜前的钱记两遍）——每帧不再重走几万行历史；写入**追加优先**（最老行超窗 30 分钟/行帽/时钟自锁探测失败才全量重写——**畸形行不触发重写**，读端本就整行丢弃，等那趟周期性重写顺带清掉即可，否则一行永不自愈的脏数据会把 30 分钟一次拖成每帧重写；稳态磁盘 churn −95%）；`/clear` 重置按单调段分别计峰，不少算
- **防御体系**：全部字段 `// ""`+数值正则守卫（含位数帽与 `10#` 十进制规范——bash 会把补零数字当八进制，且算术出错会丢弃整条复合命令、让整段甚至整块面板消失）（定长位序协议要求空字段也占一行，见 AI-GUIDE §1.1）、历史行整行形状校验、Windows jq 的 CRLF 剥离、`LC_ALL=C.UTF-8` 字符计宽、NBSP 对齐垫充（防 VSCode 终端吞空格）、行首 `\e[0m` 防宿主样式渗染、非零退出=白屏的红线
...
bash test.sh --assert   # 289 项断言（CI 用，含性能门槛与三十四轮对抗审查回归组+配色断言）
```
...
7. **自愈层（可选，Windows）**：`statusline-watchdog.ps1` 由计划任务每 2 分钟清理挂死 >90s 的渲染 bash，并兜底回收「不在注册中且活过 5 分钟」的面板 daemon（未注册的卡死实例是其余回收路径够不着的唯一盲区）（阈值从 30s 提高：后台脱离的 usage/CI 刷新子壳继承同一命令行，curl -m 5 / gh 可能合法跑过 30s）（fork 枯竭下的兜底）；**必须经 `statusline-watchdog.vbs`（wscript）拉起**——计划任务直接跑 powershell 会在 `-WindowStyle Hidden` 生效前闪一下控制台窗口。两文件复制到 `~/.claude/`（vbs 经 `%USERPROFILE%` 运行时解析路径，任意用户开箱即用、无需改内容；**ps1 必须带 UTF-8 BOM、vbs 必须保持纯 ASCII**——wscript 按系统 ANSI 码页解析，UTF-8 中文注释会在 GBK 下吞换行、把真代码吞进注释，实锤过每 2 分钟弹错误框，test.sh 有断言防复发）后：`schtasks /Create /SC MINUTE /MO 2 /TN claude-statusline-watchdog /TR "wscript.exe C:\Users\<你>\.claude\statusline-watchdog.vbs"`。
8. **面板常驻 daemon**：subagentStatusLine 命令指向 `statusline-panel-hook.sh`——钩子只做"倒载荷 + 秒回上一帧缓存"（纯内建，稳态零派生，延迟≈bash 启动的毫秒级），真正的渲染由 `statusline-panel-daemon.sh` 异步完成（内容滞后一拍 ~5s，对累计 token/用时无感知）。宿主重画面板是"先默认行、钩子返回才替换"，钩子延迟=默认行闪烁窗口——daemon 化把它从整段渲染耗时（~300ms）压到毫秒级。缓存键=载荷首任务 id（并发会话任务集不相交，天然各用各的缓存）；面板缓存首行为渲染纪元、钩子拒供 >60s 陈旧帧（daemon 起不来时诚实降级为默认行而非永久回放冻结帧）；daemon 单实例（noclobber 抢占；pid 文件**四行**协议 pid+心跳+Windows pid+在途渲染子进程 pid（第 3 行取自 /proc/self/winpid，供 PowerShell 看门狗精确识别；第 4 行是够到卡死渲染子进程的唯一把手，且是**空格分隔的列表**（当前在途的 + 连 taskkill 都没杀掉的幸存者，消费者必须迭代）——daemon 由 `( bash … & )` 无作业控制派生、自己不是进程组长，杀「它的组」是 ESRCH 空转，而渲染子进程在 `set -m` 下自成一组），daemon 墙钟 5s 原子刷新心跳、钩子探活与接管判定统一"kill -0 + 心跳 60s 内"（install 的回收**刻意不看心跳**：卡死实例心跳必然陈旧，按"新鲜才杀"恰好把最该回收的排除在外，它改为身份确认后无条件回收注册者）——残留 pid 被系统回收给无关进程时不再死锁或误杀；陈旧 pid 接管走"删除+独占重建"而非裸覆写，退出删除先验证 pid 归属——并发接管不再互踩）、无活 2 分钟自灭、死了由下一拍钩子拉起；**挂死的渲染子进程由 daemon 自身硬超时截杀跳帧**（默认 15s，`STATUSLINE_PANEL_RENDER_TIMEOUT` 可调，约为正常渲染的 50 倍——此前无超时，子进程一挂 daemon 即永久卡死、探活却始终"存活"，全部会话面板冻结在旧帧且默认安装无任何恢复路径；第 7 条看门狗现按**心跳陈旧**判据纳入 daemon 回收，钩子发现「pid 活着但心跳冻结」会先杀后拉，daemon 另有绝对寿命闸 1 小时（`STATUSLINE_PANEL_DAEMON_MAX_LIFE`）——2026-08-14 实测过一次事故：一个卡死实例使钩子每 ~65 秒拉起一个新 daemon，累计 **78 个孤儿常驻烧掉 22 CPU 小时**，当时无任何组件负责回收；另注意 **cygwin 的 `ps` 看不到全部实例**，回收必须走 Windows 进程表；2026-08-15 又抓到同型的第二、第三次，第三次才找到根因：**cygwin 的 `kill -9` 杀不掉卡在 cygwin DLL 里的进程**（返回 0 而进程纹丝不动），必须按 `/proc/<pid>/winpid` 升级到 `taskkill //F`——钩子的回收、daemon 的渲染超时、daemon 的让位退出这三条路现在都这么做；回收判据也从「命令行以脚本名结尾」改为「脚本必须是 bash 的第一个参数」，否则带参数启动的实例（如 `--once`）对所有回收路径隐身，实测两个这样的卡死实例烧掉 14 CPU 小时）——**回收失败时钩子不再拉新**（那才是每分钟净增一个不死实例的放大器）：此时面板会一直停在宿主默认行，直到第 7 条的看门狗**按「注册者心跳陈旧 >180s（心跳取 pid 第 2 行与 `hb` 旁路的较新者——只看第 2 行会在 fork 枯竭时把健康 daemon 判死，见 AI-GUIDE §4）」那条分支**删掉注册（钩子下一拍才敢拉新）——注意解锁靠的是**删注册**而不是进程真被杀死，且那个杀不掉的实例仍在注册中，所以「不在注册中且活过 5 分钟」的孤儿分支按构造正好跳过它；最坏恢复时延 ≈ 心跳陈旧 180s + 计划任务 ≤2 分钟节拍，**故强烈建议装上可选的看门狗层**（`--with-watchdog`），否则这种极端情形下面板不会自行恢复。其余环节仍是全链路自愈、无需手工管理。状态目录 `~/.claude/statusline-panel.d/`。
