...
if [ -n "$TEST_NOW_EPOCH" ]; then now_epoch="$TEST_NOW_EPOCH"; else printf -v now_epoch '%(%s)T' -1; fi
echo "DEBUG now_epoch=$now_epoch" >&2
  ...
  if [ -n "$repo_owner" ] && [ -n "$repo_name" ] && command -v gh >/dev/null 2>&1; then
    ...
    if [ -f "$ci_cache_file" ]; then
      ...
      if [ "$ci_cache_repo" = "$ci_repo_pr" ] && [ "$ci_cache_pr" = "$pr_number" ] && [[ "$ci_cache_epoch" =~ ^[0-9]+$ ]]; then
        ...
        echo "DEBUG ci_age=$ci_age ci_state='$ci_state'" >&2
      fi
    fi
    if [ -n "$ci_state" ]; then
      ...
      echo "DEBUG CI-REFRESH-SPAWNED" >&2
      (
...
if [ -f "$usage_cache_file" ]; then
  ...
  if [[ "$usage_epoch" =~ ^[0-9]+$ ]]; then
    ...
    echo "DEBUG usage_epoch=$usage_epoch usage_age=$usage_age usage_needs_refresh=$usage_needs_refresh" >&2
    if [ "$usage_age" -le 1800 ]; then
      echo "DEBUG cached-numbers-path-taken (usage_age<=1800)" >&2
...
echo "DEBUG about-to-test-refresh-gate usage_needs_refresh=$usage_needs_refresh" >&2
if [ "$usage_needs_refresh" -eq 1 ] && command -v curl >/dev/null 2>&1; then
  echo "DEBUG REFRESH-BLOCK-ENTERED" >&2
