...
4. **Windows 陷阱**：**MSYS bash 按 UTF-16 计数**——一个星平面字符（emoji、CJK 扩展 B）在 `${#s}` / `${s:i:1}` 眼里是**两个字符**（代理对各一半）。凡按字符切串（本项目只有描述与身份两处截断），切点落在代理对中间就会吐出 4 字节序列的前 3 字节 = **非法 UTF-8**：宿主严格解码会整行丢弃（那个代理永远退回默认渲染），宽容解码则是替换字符 + 宽度记账错位。截断后必须检查末字符是否为孤立高代理（0xD800-0xDBFF，即 55296-56319），是则回退一格。Linux/glibc 上同一个字符 `${#s}` 为 1，不会触发——只在 Windows 主战场上炸。Windows 版 jq 输出 CRLF。`$(...)` 命令替换在 MSYS bash 会剥掉尾部 `\r`，**但 `mapfile`/`read` 逐行读不会**——凡逐行读 jq 输出，必须管过 `tr -d '\r'`，否则数值校验全部静默失败。**`.ps1` 必须带 UTF-8 BOM**（PowerShell 5.1 对无 BOM 的 .ps1 按系统 ANSI 码页解码，本机 GBK——与下面 vbs 同一个坑，只是 CRLF 恰好挡住了它；test.sh 有 BOM 断言）。**`.vbs` 必须纯 ASCII + CRLF**：wscript 按系统 ANSI 码页（本机 GBK）解析 vbs，行尾 UTF-8 多字节字符的末字节若落在 DBCS 前导区（如 `。` 的 0x82）会把换行符当尾字节吞掉、注释一路吞并下一行——生产实锤：`Set sh = CreateObject` 整行被吞进注释，计划任务每 2 分钟弹一次"缺少对象: 'sh'"错误框（test.sh 已有纯 ASCII 断言防复发）。
...
**首选**：仓库根目录 `bash test.sh --assert` —— 149 项断言（四行结构、各新段存在性、stash 段显隐、子代理 10s 采样、双层花费存储、追加优先裁剪触发、面板钩子 spool/缓存秒回/daemon 渲染、安装器本地模式/合并/幂等、**NBSP 分隔符列对齐（含断言活性自证——分隔符样式再变会红而不是断言静默失效）**、TSV 列序、空列裁剪、性能 <3s 门槛，及**对抗审查回归组**：@tsv 反斜杠单次反解码、C0 清洗不串位不坏 JSON、session_name 内嵌换行不降级、ctx 回退负值夹取/垃圾整段消失、主目录兄弟目录边界、daemon 挂死子进程超时截杀、时钟回拨不冻结采样、~500KiB transcript <2.5s 性能门、看门狗 vbs 纯 ASCII，及第 2/3 轮回归组：垃圾限额段消失、OSC 注入剥除、未来日 rollup 不虚增、空 dir 无 git 段、数字 id 无键、空渲染保帧、usage 失败负缓存且测试不碰真实 ~/.claude、坏帧连击清缓存诚实降级、陈旧心跳接管驱逐、毫秒 resets_at 半段消失；以及第 13 轮回归组：settled 日折叠与终态行合并均减午夜基线且 `_agg` 写满 7 列、溢出合并按位读 last_epoch（3h 内活行不被并走）、跨午夜会话 today 段不消失、daemon 将在途渲染子进程发布在 pid 第 4 行且钩子精确回收它），全 PASS 即基本达标；以下手工清单用于断言未覆盖的细节。

主状态栏（`fixtures/` 三份 + 状态文件人工历史；以下期望串均为隔离环境实测）：
- **full.json**：**四行**齐全；电池 `ctx ███░│ 66% 70k/200k`（**末格恒为 80% 压缩线 `│`**，仅满电 5 格时才无标记；占用 69671=68471+1200→70k，非 68k！）；`cache 92.3%` 绿（一位小数）；第 4 行实测为 `5h 37% 7d 12% | » my-session`——**没有 `·t…%` 游标、没有 `→重置时刻`、也没有 `wk`**，与下一条 low-context 同因：夹具的 resets_at 硬编码在 2026-02-02，早已落在 §1.2 第 13 条的合理区间外，按规格这两个半段就该整段消失；`wk` 需要 usage 缓存，隔离环境不可能有。要看游标/重置时刻，照 test.sh 现场把 resets_at 注入成 `now+7200` / `now+259200`。
