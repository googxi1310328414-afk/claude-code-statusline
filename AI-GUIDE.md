...
| 7 | 上下文 | `remaining_percentage` 存在才显示。格式：`[!]ctx N% left [Xk/Yk]`。阈值判断用**整数截断**（`${v%%.*}`，等价于 floor）：<20 → 前缀独立**亮红** `!` 且百分比**亮红**；20–49 → 百分比**黄**；≥50 → 百分比**绿**。`ctx`、`left` 标签**灰**。百分比显示值用 `printf '%.0f'` 四舍五入（显示与阈值判断分离，允许 19.6 显示为 20% 但仍触发警告）。token 数：`total_input_tokens` 与 `context_window_size` 都存在且为纯数字才显示，各自 `(n+500)/1000` 整数运算取 k：`68k` **白** + `/200k`（含斜杠）**灰**。 |
| 8 | 花费 | `.cost.total_cost_usd` 存在才显示。`$` **灰**；金额 `printf '%.2f'`，色由整数部分定：<1 **灰**、1–4 **黄**、≥5 **亮红**。 |
| 9 | 行数 | added/removed 任一存在就显示（缺的按 0）：`+A` **绿** + `/`（不着色）+ `-R` **红**。 |
| 10 | 限额 | 两个窗口各自独立可选，窗口间以普通空格连接。每窗口：标签 `5h`/`7d` **灰**；百分比 `printf '%.0f'`，色由 floor 值定：<50 **绿**、50–79 **黄**、≥80 **亮红**；`resets_at` 通过 `^[0-9]+$` 校验后才交给 date——5h 窗口 `→HH:MM`（本地时间）、7d 窗口 `→MM-DD`，箭头和时间**灰**。 |
...
- **full.json**（全字段）：11 段齐全、顺序正确；`68471→68k`、`0.4231→$0.42`、百分比四舍五入正确。
- **minimal.json**（新会话、主目录、无仓库）：只剩 `时间 | 模型 | ~ | $0.00 | +0/-0`，无多余分隔符。
- **low-context.json**（剩 12.3%）：`!` 前缀出现且亮红；`~\proj\webapp` 父/末级两色；`$1.07` 黄；`5h 82%` 亮红带重置时间、`7d 45%` 绿不带重置时间。
