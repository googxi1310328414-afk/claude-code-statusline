...
11. 花费：（**today 半段有显示门**：仅当 `today_total - 本会话今日贡献 >= 1 分` 才渲染，即「今天还有别的会话花过钱」；单会话时它与紧邻的金额段完全重复，故整段消失——复刻时不可无条件渲染 today）`$` 不着色（终端默认色）+ 金额 `printf '%.2f'` 档色（整数部分 <1灰/1–4黄/≥5亮红）+ 空格 + `$X.X/h` 速率（历史文件 60 分钟窗口，纯 bash 分转换：整数部分*100+前两位小数，需时距 ≥120s 且差值非负，`cents*3600/span` 得每小时分数，按整美元档色同金额）。速率半独立可缺。
